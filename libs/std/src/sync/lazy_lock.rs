    ...
    pub fn into_inner(mut this: Self) -> Result<T, F> {
        ...
        match state {
            ...
            state => {
                ...
                match state {
                    OnceExclusiveState::Incomplete => Err(ManuallyDrop::into_inner(data_f(data))),
                    OnceExclusiveState::Complete => Ok(ManuallyDrop::into_inner(data_value(data))),
...
impl<T, F> Drop for LazyLock<T, F> {
    fn drop(&mut self) {
        match self.once.state() {
            OnceExclusiveState::Incomplete => unsafe { ManuallyDrop::drop(data_f_mut(self.data.get_mut())) },
