...
impl<T: ?Sized, A: Allocator> Arc<T, A> {
    ...
    /// Like [`from_raw_in`], except that this function accepts an `ArcInner<T>`
    /// pointer instead of a `T` pointer, with an allocator. This is meant to serve as a
    /// replacement for [`from_raw_in`] (to be used in conjunction with
    /// [`into_inner_raw_with_allocator`]) that is feasible for `crucible-mir` to simulate.
    #[inline]
    #[unstable(feature = "allocator_api", issue = "32838")]
    pub unsafe fn from_inner_raw_in(ptr: *const ArcInner<T>, alloc: A) -> Self {
        unsafe { Arc::from_ptr_in(ptr as *mut ArcInner<T>, alloc) }
    }

    /// Like [`into_raw_with_allocator`], except that this function returns an
    /// `ArcInner<T>` pointer instead of a `T` pointer. This is meant to serve as a
    /// replacement for [`into_raw_with_allocator`] (to be used in conjunction with
    /// [`from_inner_raw_in`]) that is feasible for `crucible-mir` to simulate.
    #[must_use = "losing the pointer will leak memory"]
    #[unstable(feature = "allocator_api", issue = "32838")]
    pub fn into_inner_raw_with_allocator(this: Self) -> (*const ArcInner<T>, A) {
        let this = mem::ManuallyDrop::new(this);
        let ptr = this.ptr.as_ptr() as *const ArcInner<T>;
        let alloc = unsafe { ptr::read(&this.alloc) };
        (ptr, alloc)
    }
