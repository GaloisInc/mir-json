...
`crucible-mir` is not currently able to simulate the
`{Arc,Rc}::{from,into}_raw` functions. This is because they subtract a
constant offset from a `*T` pointer in order to obtain a pointer of type
`*ArcInner<T>`/`*RcInner<T>`, and then subsequently reading from it. This sort
of pointer arithmetic is permitted in Rust, but `crucible-mir`'s memory model
is not yet sophisticated enough to support this.
