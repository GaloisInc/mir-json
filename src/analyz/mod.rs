...
impl<T> ToJson for ty::Slice<T> where T : ToJson {
    fn to_json(&self, mir : &Mir) -> serde_json::Value {
        let mut j = Vec::new();
        for v in self {
...
impl ToJson for hir::Mutability {
    fn to_json(&self, _ : &Mir) -> serde_json::Value {
        json!(match self { &hir::Mutability::MutMutable => "mut", _ => "immut" })
    }
}
impl ToJson for mir::Mutability {
    fn to_json(&self, _ : &Mir) -> serde_json::Value {
        json!(match self { &mir::Mutability::Mut => "mut", _ => "immut" })
    }
}
...
impl<'a> ToJson for middle::const_val::ConstVal<'a> {
    fn to_json(&self, mir: &Mir) -> serde_json::Value {
        match self {
            &middle::const_val::ConstVal::Integral(i) if !(i.is_negative()) => json!({"kind": "int", "data": i.to_u64().unwrap()}),
            &middle::const_val::ConstVal::Bool(b) => json!({"kind": "bool", "data": b}),
            &middle::const_val::ConstVal::Function(defid, substs) => json!({"kind": "function", "fname": defid.to_json(mir), "substs": substs.to_json(mir)}),
            &middle::const_val::ConstVal::Array(ref constvals) => json!({"kind": "array", "data": constvals.to_json(mir)}),
...
impl<'a> ToJson for mir::Rvalue<'a> {
    fn to_json(&self, mir : &Mir) -> serde_json::Value {
        match self {
            ...
            &mir::Rvalue::Repeat(ref op, ref s) => json!({"kind": "repeat", "op": op.to_json(mir), "len": s.to_json(mir)}),
            &mir::Rvalue::Ref(_ , ref bk, ref l) => json!({"kind": "ref", "region": "unimplement", "borrowkind": bk.to_json(mir), "refvar": l.to_json(mir)}), // UNUSED
            &mir::Rvalue::Len(ref l) => json!({"kind": "len", "lv": l.to_json(mir)}),
            &mir::Rvalue::Cast(ref ck, ref op, ref ty) => json!({"kind": "cast", "type": ck.to_json(mir), "op": op.to_json(mir), "ty": ty.to_json(mir)}),
            &mir::Rvalue::BinaryOp(ref binop, ref op1, ref op2) => json!({"kind": "binaryop", "op": binop.to_json(mir), "L": op1.to_json(mir), "R": op2.to_json(mir)}),
            &mir::Rvalue::CheckedBinaryOp(ref binop, ref op1, ref op2) => json!({"kind": "checkedbinaryop", "op": binop.to_json(mir), "L": op1.to_json(mir), "R": op2.to_json(mir)}),
...
impl<'a> ToJson for mir::ProjectionElem<'a, mir::Operand<'a>> {
    fn to_json(&self, mir : &Mir) -> serde_json::Value {
        match self {
            ...
            &mir::ProjectionElem::Field(ref f, ref ty) => json!({"kind": "field", "field": f.to_json(mir), "ty": ty.to_json(mir)}),
...
impl<'a> ToJson for mir::Lvalue<'a> {
    fn to_json(&self, mir : &Mir) -> serde_json::Value {
        match self {
            &mir::Lvalue::Local(ref l) => json!({"kind": "local", "localvar": local_json(mir, *l) }),
            &mir::Lvalue::Static(_) => json!({"kind": "static"}), // UNUSED
            &mir::Lvalue::Projection(ref p) => json!({"kind": "projection", "data" : p.to_json(mir)})
        }
    }
}
...
impl<'a> ToJson for mir::Literal<'a> {
    fn to_json(&self, mir : &Mir) -> serde_json::Value {
        match self {
            &mir::Literal::Item {ref def_id, ref substs} => json!({"kind": "item", "def_id": def_id.to_json(mir), "substs": substs.to_json(mir)}),
            &mir::Literal::Value {ref value} => json!({"kind": "value", "value": value.to_json(mir)}),
            &mir::Literal::Promoted {ref index} => json!({"kind": "promoted", "index": index.to_json(mir)})
        }
    }
}
...
impl<'a> ToJson for mir::Statement<'a> {
    fn to_json(&self, mir : &Mir) -> serde_json::Value {
        match &self.kind {
            &mir::StatementKind::Assign(ref l, ref r) => json!({"kind": "Assign", "lhs": l.to_json(mir), "rhs": r.to_json(mir)}),
            &mir::StatementKind::SetDiscriminant{ref lvalue, ref variant_index} => json!({"kind": "SetDiscriminant", "lvalue": lvalue.to_json(mir), "variant_index": variant_index}),
            &mir::StatementKind::StorageLive(ref l) => json!({"kind": "StorageLive", "slvar": l.to_json(mir)}),
            &mir::StatementKind::StorageDead(ref l) => json!({"kind": "StorageDead", "sdvar": l.to_json(mir)}),
...
impl<'a> ToJson for mir::TerminatorKind<'a> {
    fn to_json(&self, mir : &Mir) -> serde_json::Value {
        match self {
            &mir::TerminatorKind::Goto{ref target} => json!({"kind": "goto", "target": target.to_json(mir)}),
            &mir::TerminatorKind::SwitchInt{ref discr, ref switch_ty, ref values, ref targets} => {
            ...
            &mir::TerminatorKind::Drop{ref location, ref target, ref unwind} => json!({"kind": "drop", "location": location.to_json(mir), "target" : target.to_json(mir), "unwind": unwind.to_json(mir)}),
            &mir::TerminatorKind::DropAndReplace{ref location, ref value, ref target, ref unwind} =>
...
pub fn get_def_ids(tcx: TyCtxt) -> Vec<DefId> { tcx.mir_keys(def_id::LOCAL_CRATE).iter().cloned().collect::<Vec<_>>() }
...
fn mir_info<'a,'tcx>(mir : &Mir<'a>, def_id : DefId, src : MirSource, tcx : &TyCtxt<'a, 'tcx, 'tcx>) -> Option<serde_json::Value> {
    match src {
        MirSource::Fn(_) => (),
        _ => return None
    };
    ...
    Some(json!({"name": fn_name, "args": args, "return_ty": mir.return_ty.to_json(mir), "body": body}))
...
fn mir_body<'a,'tcx>(mir : &Mir<'a>, _def_id : DefId, _src : MirSource, _tcx : &TyCtxt<'a, 'tcx, 'tcx>) -> serde_json::Value {
    ...
    for bb in mir.basic_blocks().indices() {
        ...
        //theyre not in order
        blocks.push(json!({"blockid": bb.to_json(mir), "block": mir[bb].to_json(mir)}));
    }
