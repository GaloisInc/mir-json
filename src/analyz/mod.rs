...
fn vtable_descriptor_for_cast<'tcx>(
    ...
    /// ADT, recursively produce its last field.
    fn last_field<'tcx>(
        tcx: TyCtxt<'tcx>,
        ...
        match *last_field_instantiated.kind() {
            ty::TyKind::Adt(field_adt_def, field_adt_args) => {
    ...
    match (*old_ty.kind(), *new_ty.kind()) {
        ...
        (ty::TyKind::RawPtr(old_pointee, _), ty::TyKind::RawPtr(new_pointee, _)) => {
            ...
            let trait_ref = match (*old_pointee.kind(), *new_pointee.kind()) {
                ...
                (
                    ty::TyKind::Adt(old_adt_def, old_adt_args),
                    ty::TyKind::Adt(new_adt_def, new_adt_args),
